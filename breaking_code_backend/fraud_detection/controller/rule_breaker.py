    ...
    'conditions': [
        {
            'logic': 'AND',
            'conditions': [
                {'field': 'amount', 'operator': '>', 'value': "LOOKUP(default,'mid_lookup_table')"},
                {
                    'logic': 'OR',
                    'conditions': [
                        {'field': 'account_number', 'operator': '=', 'value': "LOOKUP(default,'account_number_lookup_table')"},
                        {'field': 'country', 'operator': 'IN', 'value': ["LOOKUP(default,'country_lookup_table')", "LOOKUP(default,'country_lookup_table')"]}
                    ]
                },
...
# {"logic":"AND","conditions":[{"field":"amount","operator":"IN","value":"1"}]}


# SELECT * FROM transactions WHERE ((amount > 'LOOKUP(420,'mid_lookup_table')') AND ((country = 'LOOKUP(default,'account_number_lookup_table')') OR (country IN ('LOOKUP(default,'mid_lookup_table')', 'LOOKUP(default,'mid_lookup_table')')))) OR (name > 1000);
